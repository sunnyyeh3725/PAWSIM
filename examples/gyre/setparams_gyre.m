...
%%% grid_size       Optional grid size, either scalar or [Nx Ny].
%%%                 Default is [300 200].
%%% 
function setparams_gyre (local_home_dir,run_name,grid_size)
  ...
  if (isscalar(grid_size))
    ...
    if (numel(grid_size) ~= 2)
      error('grid_size must be either a scalar or [Nx Ny]');
    end
    Nx = grid_size(1);
    Ny = grid_size(2);
  end
